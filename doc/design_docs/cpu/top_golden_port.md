...
| 项目 | 内容 |
|------|------|
| 模块名称 | top_golden_port |
| 文件路径 | C910_RTL_FACTORY/gen_rtl/cpu/rtl/top_golden_port.v |
| 模块类型 | 单核端口定义模块 |
| 作者 | T-Head Semiconductor Co., Ltd. |
| 许可证 | Apache License 2.0 |

### 1.2 功能描述
top_golden_port 是 OpenC910 处理器的单核端口定义模块，定义了单核配置下处理器的所有外部接口。该模块作为端口模板，用于验证和文档目的。
