...
//EX4 Control
gated_clk_cell  x_ex3_vld_clk (
  .clk_in             (forever_cpuclk    ),
  .clk_out            (ex3_vld_clk       ),
  .external_en        (1'b0              ),
  .global_en          (cp0_yy_clk_en     ),
  .local_en           (ex3_vld_clk_en    ),
  .module_en          (cp0_vfpu_icg_en   ),
  .pad_yy_icg_scan_en (pad_yy_icg_scan_en)
);

assign ex3_vld_clk_en = ex3_pipedown || ex4_pipedown;
always @(posedge ex3_vld_clk or negedge cpurst_b)
begin
  if(!cpurst_b)
    ex4_pipedown <= 1'b0;
